...
struct IndexWrapper {
    IndexWrapper(const std::string &method,
            ...
            : method(method), space_type(space_type), data_type(data_type), dist_type(dist_type),
              space(SpaceFactoryRegistry<dist_t>::Instance().CreateSpace(space_type, loadParams(space_params)))
    {
        ...
        if (data_type == DATATYPE_DENSE_UINT32_VECTOR && vectSpacePtr == nullptr) {
            throw std::invalid_argument("The space type " + space_type +
                                        " is not compatible with the type DENSE_UINT32_VECTOR!");
        }
    ...
    void createIndex(py::object index_params, bool print_progress = false) {
        py::scoped_ostream_redirect stream(std::cerr, py::module::import("sys").attr("stdout"));
    ...
    py::object knnQueryBatch(py::object input, size_t k, int num_threads, bool stack) {
        if (!index) {
            throw std::invalid_argument("Must call createIndex or loadIndex before this method");
        }
        ...
        if (stack)
            return convertAllResults(results, k, false);
    ...
    py::object knnQueryAll(size_t k, int num_threads, bool stack) {
        if (!index)
            throw std::invalid_argument("Must call createIndex or loadIndex before this method");

        std::vector<std::unique_ptr<KNNQueue<dist_t>>> results(size());
        {
            py::gil_scoped_release l;

            ParallelFor(0, size(), num_threads, [&](size_t query_index, size_t threadId) {
                // Get k+1 neighbors and later, remove "self"
                KNNQuery<dist_t> knn(*space, data.at(query_index), k + 1);
                index->Search(&knn, -1);
                results[query_index].reset(knn.Result()->Clone());
            });
        }
        ...
        if (stack)
            return convertAllResults(results, k, true);

        py::list results_list;
        for (auto &result : results) {
            auto *res = result.get();
            int i = (int)res->Size() - 1;
            py::array_t<IdType> ids(i);
            py::array_t<dist_t> distances(i);

            for (--i; !res->Empty(); res->Pop()) {
                auto id = res->TopObject()->id();
                if (id != results_list.size()) {
                    if (i == -1) {
                        ...
                        for (size_t j = ids.size() - 1; j > 0; --j) {
                            ids.mutable_at(j) = ids.at(j - 1);
                            distances.mutable_at(j) = distances.at(j - 1);
                        }
                        ++i;
                    }
                    ids.mutable_at(i) = id;
                    distances.mutable_at(i) = res->TopDistance();
                    --i;
                }
            }
            results_list.append(py::make_tuple(ids, distances));
        }
        return results_list;
    }
    ...
    py::tuple convertAllResults(std::vector<std::unique_ptr<KNNQueue<dist_t>>> &results, size_t k, bool remove_self) {
        std::vector<size_t> shape{results.size(), k};
        py::array_t<IdType> ids(shape);
        py::array_t<dist_t> distances(shape);
        size_t i = 0;
        for (auto &result : results) {
            auto *res = result.get();
            if (res->Size() != k + (size_t)remove_self)
                throw std::runtime_error("got unexpected number of neighbors");
            for (int j = (int)k - 1; !res->Empty(); res->Pop()) {
                auto id = res->TopObject()->id();
                if (!remove_self || id != i) {
                    if (j == -1) {
                        // This means that 'self' is not among the neighbors.
                        ...
                        for (size_t l = k - 1; l > 0; --l) {
                            ids.mutable_at(i, l) = ids.at(i, l - 1);
                            distances.mutable_at(i, l) = distances.at(i, l - 1);
                        }
                        ++j;
                    }
                    ids.mutable_at(i, j) = id;
                    distances.mutable_at(i, j) = res->TopDistance();
                    --j;
                }
            }
            ++i;
    ...
    const Object * readObject(py::object input, int id = 0, bool memory_only = true) {
        switch (data_type) {
            case DATATYPE_DENSE_VECTOR: {
                ...
                return vectSpacePtr->CreateObjFromVect(id, memory_only? -1 : -2, tempVect);
                // This way it will not always work properly
                //return new Object(id, -1, temp.size() * sizeof(dist_t), temp.data(0));
            }
            case DATATYPE_DENSE_UINT32_VECTOR: { // Temp hack
                py::array_t<uint32_t , py::array::c_style | py::array::forcecast> temp(input);
                std::vector<uint32_t> tempVect(temp.data(0), temp.data(0) + temp.size());
                auto vectSpacePtr = reinterpret_cast<VectorSpace<dist_t>*>(space.get());
                return vectSpacePtr->CreateObjFromUint32Vect(id, memory_only? -1 : -2, tempVect);
            }
    ...
    size_t readObjectVector(py::object input, ObjectVector * output,
                            py::object ids_ = py::none(), bool memory_only = true) {
        std::vector<int> ids;
        if (!ids_.is_none()) {
        ...
        if (py::isinstance<py::list>(input)) {
            py::list items(input);
            for (size_t i = 0; i < items.size(); ++i) {
                output->push_back(readObject(items[i], ids.size() ? ids.at(i) : i, memory_only));
            }
            return items.size();
            ...
            for (size_t row = 0; row < rows; ++row) {
                ...
                output->push_back(vectSpacePtr->CreateObjFromVect(id, memory_only? -1 : -2, tempVect));
                //this way it won't always work properly
                //output->push_back(new Object(id, -1, features * sizeof(dist_t), items.data(row)));
            }
            return rows;
        } else if (data_type == DATATYPE_DENSE_UINT32_VECTOR) { // Temp hack
            // allow numpy arrays to be returned here too
            py::array_t<uint32_t, py::array::c_style | py::array::forcecast> items(input);
            auto buffer = items.request();
            if (buffer.ndim != 2) throw std::runtime_error("data must be a 2d array");

            size_t rows = buffer.shape[0], features = buffer.shape[1];
            std::vector<uint32_t> tempVect(features);
            auto vectSpacePtr = reinterpret_cast<VectorSpace<dist_t>*>(space.get());
            for (size_t row = 0; row < rows; ++row) {
                int id = ids.size() ? ids.at(row) : row;
                const uint32_t* elemVecStart = items.data(row);
                std::copy(elemVecStart, elemVecStart + features, tempVect.begin());
                output->push_back(vectSpacePtr->CreateObjFromUint32Vect(id, memory_only? -1 : -2, tempVect));
            }
            return rows;
            ...
            for (int rowid = 0; rowid < indptr.size() - 1; ++rowid) {
                ...
                for (int i = indptr.at(rowid); i < indptr.at(rowid + 1); ++i)
                    sparse_items.push_back(SparseVectElem<dist_t>(indices.at(i), sparse_data.at(i)));
    ...
    py::object writeObject(const Object * obj) {
        switch (data_type) {
            ...
            case DATATYPE_DENSE_UINT32_VECTOR: {
                auto vectSpacePtr = reinterpret_cast<VectorSpace<dist_t>*>(space.get());
                py::list ret;
                const auto* values = reinterpret_cast<const uint32_t *>(obj->data());
                size_t elemQty = vectSpacePtr->GetElemQty(obj);
                for (size_t i = 0; i < elemQty; ++i) {
                    ret.append(py::cast(values[i]));
                }
    ...
    size_t addDataPoint(int id, py::object input) {
        data.push_back(readObject(input, id, false));
        return data.size() - 1;
    }

    size_t addDataPointBatch(py::object input, py::object ids = py::none()) {
        return readObjectVector(input, &data, ids, false);
    }
    ...
    py::array_t<dist_t> pairwiseDistance(py::object indices, int num_threads) const {
        py::array_t<size_t> indarray(indices);
        size_t N = indarray.size();
        size_t M = 2 * N - 1;
        size_t M2 = M * M;
        py::array_t<dist_t> dist(N * (N - 1) / 2);
        {
            ...
            ParallelFor(0, dist.size(), num_threads, [&](size_t k, size_t threadId) {
                size_t i = (size_t)((M - sqrt(M2 - 8 * k)) / 2);
                size_t j = i * (i + 3) / 2 + k - i * N + 1;
                dist.mutable_at(k) = space->IndexTimeDistance(data.at(indarray.at(i)), data.at(indarray.at(j)));
            });
        }
        return dist;
    }
...
PYBIND11_PLUGIN(nmslib) {
    ...
    py::enum_<DataType>(m, "DataType")
            ...
            .value("DENSE_UINT32_VECTOR", DATATYPE_DENSE_UINT32_VECTOR)
...
void exportIndex(py::module *m) {
    ...
    py::class_<IndexWrapper<dist_t>>(*m, index_name.c_str())
            ...
            .def("knnQueryBatch", [](IndexWrapper<dist_t> *self, py::object input, size_t k, int num_threads, bool stack) {
                     if (input.is_none())
                         return self->knnQueryAll(k, num_threads, stack);
                     return self->knnQueryBatch(input, k, num_threads, stack);
                 },
                 py::arg("input") = py::none(), py::arg("k") = 10, py::arg("num_threads") = 0, py::arg("stack") = false,
                 "Performs multiple queries on the index, distributing the work over \n"
                 "a thread pool.\n"
                 "If an input is not given, the query is performed on the entire index.\n\n"
                 "Parameters\n"
                 "----------\n"
                 "input: list optional\n"
                 "    A list of objects to query for\n"
                 "k: int optional\n"
                 "    The number of neighbours to return\n"
                 "num_threads: int optional\n"
                 "    The number of threads to use\n"
                 "stack: bool optional\n"
                 "    Whether to return the results as stacked numpy arrays\n"
                 "\n"
                 "Returns\n"
                 "----------\n"
                 "list (if `stack` is false):\n"
                 "   A list of tuples of (ids, distances)\n"
                 "tuple (if `stack` is true):\n"
                 "   A pair of stacked (ids, distances)\n")
            ...
            .def("pairwiseDistance", &IndexWrapper<dist_t>::pairwiseDistance,
                 py::arg("indices"), py::arg("num_threads") = 0)
...
void exportLegacyAPI(py::module *m) {
    ...
    m->def("knnQueryBatch", [](py::object self, int num_threads, int k, py::object queries) {
        py::list results = self.attr("knnQueryBatch")(queries, k, num_threads, false);
